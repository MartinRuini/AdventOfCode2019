...
def fetchInstruction(iPointer, list_):
    #print(f"\n\n#fetching, ip={iPointer}, instr: {list_[iPointer]}")
    modes, instr = parseInstruction(list_[iPointer])
    #print(f'modes: {modes}, instr: {instr}')
    #print(list_[iPointer])
    #instr = list_[iPointer]
    if instr == 99:
        return -1
    ...
    if modes[0] == 0:
        param1 = int(list_[loc1])
    else:
        param1 = loc1
    if instr == 1 or instr == 2 or instr == 5 or instr == 6 or instr == 7 or instr == 8:
        if modes[1] == 0:
            param2 = int(list_[loc2])
        else:
            param2 = loc2
        if modes[2] == 1:
            param3 = loc3
    ...
    if instr == 1:      # add
        ...
        res = input("Please enter the ID: ")
        list_[loc1] = res
        #print(f'scritto: {res} in {loc1}')
        return iPointer + 2
    elif instr == 4:
        #print(f'IPOINTER:{iPointer}')
        print(f'{param1}')
        return iPointer + 2
